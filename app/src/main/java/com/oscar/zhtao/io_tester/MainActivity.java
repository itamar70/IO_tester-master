package com.oscar.zhtao.io_tester;
...
public class MainActivity extends AppCompatActivity {
    ...
    private void runIOTest(StorageLocation location, long loopCount, long delay) {
        ...
        if (location == MainActivity.StorageLocation.PRIVATE) {
            file = new File(getApplicationContext().getFilesDir(), TestFilename);
        } else if (location == MainActivity.StorageLocation.EXTERNAL) {
            file = new File(Environment.getExternalStorageDirectory(), TestFilename);
        } else if (location == MainActivity.StorageLocation.SECONDARY) {
            file = new File(SecondaryExternal, TestFilename);
        } else {
            dprintln("Unsupported storage location: " + location);
            return;
        }
        Log.i("runIOTest", "runIOTest PID = " + String.valueOf(Process.myTid()));
        byte buf4K[] = new byte[4096];
        Random rand = new Random();

        rand.nextBytes(buf4K);
        rand = null;
        //byte buf1K[] = new byte[1024];
        //File file = new File(Environment.getExternalStorageDirectory(), "test");

        Dynamo dynamo = new Dynamo(file, CONF_FILE_SIZE, Dynamo.IoEngine.RANDOMACCESS, Dynamo.SyncType.RWD, Dynamo.Direction.OUTPUT);
        dynamo.initialize();
        ...
        try {
            ...
            gui_tv_state.post(new Runnable() {
                @Override
                public void run() {
                    gui_tv_state.setText("Running from " + startTime);
                }
            });
            ...
            while (true) {
                ...
                offset = dynamo.nextRand4KOffset();
                dynamo.doWrite(offset, buf4K);
                sync_count += 1;
                ...
                if ((i % UI_UPDATE_INTERVAL) == 0) {
                    ...
                    final double bw = 4096*i*1000000000.0/(endTime-startTime)/1024;
                    final double intervalBW = 4096*UI_UPDATE_INTERVAL*1000000000.0/(endTime-intervalStartTime)/1024;
                    final long iter = i;

                    gui_tv_con.post(new Runnable() {
                        @Override
                        public void run() {
                            println("Runtime [" + iter + "]: " + (endTime-startTime)/1000000000.0 + "s, bw = " + bw + "KiB/s" + " intervalBW " + intervalBW + " KiB/s");
                        }
                    });
                    Log.i("runIOTest", "Runtime [" + iter + "]: " + (endTime-startTime)/1000000000.0 + "s, bw = " + bw + "KiB/s" + " intervalBW " + intervalBW + " KiB/s");
                    intervalStartTime = System.nanoTime();
                }
    //                File file = new File(this.getFilesDir(), "abs." + str);
    ...
    //                out.close();
            ...
            final double bw = 4096*i*1000000000.0/(endTime-startTime)/1024;
            Log.i("runIOTest", "Final Runtime: " + (endTime-startTime)/1000000000.0 + "s, bw = " + 4096*i*1000000000.0/(endTime-startTime)/1024 + "KiB/s");
            wakeLock.release();
            gui_tv_con.post(new Runnable() {
                @Override
                public void run() {
                    println("Final Runtime: " + (endTime-startTime)/1000000000.0 + "s, bw = " + bw + "KiB/s");
                    gui_tv_state.setText("Runtime: " + (endTime-startTime)/1000000000.0 + "s, bw = " + bw + "KiB/s");
                }
            });
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        wakeLock = powerManager.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "IO_TESTER");
...
//                Uri.parse("android-app://com.oscar.zhtao.io_tester/http/host/path")
//        );
